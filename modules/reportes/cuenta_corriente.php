...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($pid_post > 0) {
        ...
        try {
            ...
            $db->rollBack();
            error_log('cc_viajes guardar: ' . $e->getMessage());
        }
    }
...
if ($proveedor_id > 0) {
    ...
    // Remitos en stock durante el período
    $stmt = $db->prepare("
    ...
    // Camiones guardados para este mes
    ...
    while ($cursor <= $finDt) {
        ...
        // Costo de distribución según modo
        $costo_viaje = null;
...
// Columnas fijas: fecha, concepto, remito/cliente, pal.entrada, pal.salida, camiones(opt), stock, posiciones
$ncols_base  = 8 + ($modo_camion ? 1 : 0);
$ncols_extra = ($con_pos ? 1 : 0) + ($con_viaje ? 1 : 0) + ($con_total ? 1 : 0);
$ncols       = $ncols_base + $ncols_extra;
...
<head>
    ...
    <style>
        ...
        .row-dia td, .row-viaje td { border-top: 2px solid #dee2e6 !important; }
        ...
        .input-cam { width: 60px; text-align: center; font-weight: 700;
...
<div class="container-fluid py-3 px-4" style="max-width:1200px">
    ...
    <form method="GET" class="card mb-3 no-print">
        <div class="card-body py-3">
            <div class="row g-2 align-items-end">
                ...
                <!-- Distribución: toggle modo + precio -->
                <div class="col-sm-4 col-lg-3">
                    <label class="form-label form-label-sm fw-semibold mb-1">
                    ...
                    <div class="input-group input-group-sm">
                        <select name="precio_modo" class="form-select form-select-sm" style="max-width:130px"
                                onchange="this.form.querySelector('[name=precio_viaje]').placeholder = this.value==='camion' ? '$ / camión' : '$ / pallet'">
    ...
    <div class="d-none d-print-block mb-3">
        ...
        <div class="text-muted fs-6">
            ...
            <?php if ($con_viaje): ?> &nbsp;·&nbsp; Distribución $<?= number_format($precio_viaje, 2, ',', '.') ?>/<?= $modo_camion ? 'camión' : 'pallet' ?><?php endif; ?>
        </div>
        <hr>
    </div>
    ...
    <div class="row g-3 mb-4">
            ...
            <div class="card h-100">
                <div class="card-body text-center py-3">
                    <div class="text-muted small text-uppercase fw-semibold mb-1">
                        <?= $modo_camion ? 'Camiones' : 'Pal. distrib.' ?>
                    </div>
        ...
        <div class="col-6 col-md-2">
            <div class="card h-100" style="border:2px solid #198754 !important">
                <div class="card-body text-center py-3">
    ...
    <!-- ── Tabla + form para guardar camiones (POST) ──────── -->
    ...
    <div class="card">
        <div class="card-header bg-white d-flex justify-content-between align-items-center py-2">
            ...
            <?php if ($modo_camion): ?>
            <button type="submit" class="btn btn-warning btn-sm no-print">
        ...
        <table class="table table-sm align-middle mb-0" id="tabla-cc">
            <thead>
                <tr>
                    ...
                    <?php if ($con_viaje): ?>
                    <th class="text-end">
                        <?= $modo_camion ? '$ Distrib. (camión)' : '$ Distrib. (pallet)' ?>
                    </th>
                    <?php endif; ?>
            ...
            <?php // ── Filas de ingreso (una por remito, con detalle) ── ?>
            ...
            <tr class="row-entrada">
                ...
                <td></td>
                <?php if ($modo_camion): ?><td></td><?php endif; ?>
                <td></td><td></td>
            ...
            <?php // ── Fila de entrega/cierre del día ───────────────── ?>
            <tr class="<?= $tiene_salidas ? 'row-viaje' : 'row-dia' ?>">
                <td class="small"><?= !$tiene_entradas ? $fmtFecha : '' ?></td>
                <td>
                ...
                <td class="text-muted small fst-italic">
                    <?= (!$tiene_movs && $info['stock'] > 0) ? 'Stock en depósito' : '' ?>
                </td>
                <td></td>
                ...
                <td class="text-center no-print">
                    <?php if ($tiene_salidas): ?>
                    <input type="number" name="cam[<?= $dia ?>]"
                           class="input-cam"
                           value="<?= $info['camiones'] ?: '' ?>"
                ...
                <td class="text-end <?= $tiene_salidas ? 'col-viaje' : 'text-muted' ?>">
                    <?php if ($tiene_salidas && $info['costo_viaje'] !== null): ?>
                        <?php if ($modo_camion && $info['camiones'] === 0): ?>
                        <span class="text-muted fst-italic small">sin camiones</span>
                        <?php else: ?>
            ...
            <tr class="subtotal-row">
                <td colspan="<?= 7 + ($modo_camion ? 1 : 0) ?>" class="text-end pe-2 text-muted small text-uppercase">Subtotal almacenaje</td>
            ...
            <tr class="subtotal-row">
                <td colspan="<?= 4 + ($modo_camion ? 1 : 0) ?>" class="text-end pe-2 text-muted small text-uppercase">Subtotal distribución</td>
                <td class="text-end col-viaje">
                    <?= $modo_camion
                        ? $datos['total_camiones'] . ' cam.'
                        : fmtPal($datos['total_pal_viajes']) . ' pal.' ?>
                </td>
                <td colspan="3"></td>
                <?php if ($con_pos):   ?><td></td><?php endif; ?>
            ...
            <tr class="total-row">
                ...
                <td colspan="<?= $ncols ?>" class="text-end pe-3">
                    <?= strtoupper($meses[$mes]) ?> <?= $anio ?>
                    — <?= number_format($datos['total_posiciones'], 1) ?> posiciones
                </td>
